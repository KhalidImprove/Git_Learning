...
class CanoeSync(object):
    ...
    def LoadTestSetup(self, testsetup):
        ...
        #get the number of test environments in the configuration
        TestEnvCount = self.TestSetup.TestEnvironments.Count
        # check if the configuration has test environments
        if (TestEnvCount != 0):
            # delete all the already-existing test environments from the configuration
            for item in self.TestSetup.TestEnvironments:
                self.TestSetup.TestEnvironments.Remove(item.Name,False)
        waitsleep(1)
    ...
    def LoadTestConfiguration(self, testcfgname, testunits):
        # get the number of test configuration
        TestConfigCount = self.App.Configuration.TestConfigurations.Count
        # check if the CANoe configuration contains any test configuration
        if(TestConfigCount !=0 ):
            # delete any test configuration defined in CANoe
            for item in self.App.Configuration.TestConfigurations:
                self.App.Configuration.TestConfigurations.Remove(item.Name)
